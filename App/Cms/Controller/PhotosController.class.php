...
class PhotosController extends AdminCoreController {
	...
	protected function _search() {
		$map = array ();
		$post_data=I('post.');
		/* 名称：对象 字段：obj_id 类型：num*/
		if($post_data['s_obj_id']!=''){
			$map['obj_id']=$post_data['s_obj_id'];
		}
		/* 名称：地址 字段：href 类型：pictures*/
		if($post_data['s_href']!=''){
			$map['href']=$post_data['s_href'];
		}
	...
	public function add(){
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					action_log('Add_Photos', 'Photos', $result);
	...
	public function edit(){
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					action_log('Edit_Photos', 'Photos', $post_data['id']);
					$this->success ( "操作成功！",U('index'));
				}else{
					$error = $this->Model->getError();
					$this->error($error ? $error : "操作失败！");
	...
	public function del(){
		...
		if(!$res){
			...
			action_log('Del_Photos', 'Photos', $id);
			$this->success('删除成功！');
		}
	}
}