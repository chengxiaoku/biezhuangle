...
class BannerController extends AdminCoreController {
	...
	protected function _search() {
		...
		/* 名称：封面图 字段：cover_img 类型：pictures*/
		if($post_data['s_cover_img']!=''){
			$map['cover_img']=$post_data['s_cover_img'];
		}
		return $map;
	}
	...
	public function add(){
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					action_log('Add_Banner', 'Banner', $result);
	...
	public function edit(){
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					action_log('Edit_Banner', 'Banner', $post_data['id']);
					$this->success ( "操作成功！",U('index'));
				}else{
					$error = $this->Model->getError();
					$this->error($error ? $error : "操作失败！");
	...
	public function del(){
		...
		if(!$res){
			...
			action_log('Del_Banner', 'Banner', $id);
			$this->success('删除成功！');
		}
	}
}
