...
class DesignController extends AdminCoreController {
	...
	public function add(){
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					action_log('Add_Design', 'Design', $result);
	...
	public function edit(){
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					action_log('Edit_Design', 'Design', $post_data['id']);
					$this->success ( "操作成功！",U('index'));
				}else{
					$error = $this->Model->getError();
					$this->error($error ? $error : "操作失败！");
