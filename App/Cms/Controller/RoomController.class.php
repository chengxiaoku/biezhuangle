...
class RoomController extends AdminCoreController {
	...
	protected function _search() {
		...
		/* 名称：选中图标 字段：icon_on 类型：pictures*/
		if($post_data['s_icon_on']!=''){
			$map['icon_on']=$post_data['s_icon_on'];
		}
		return $map;
	}
	...
	public function add(){
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					action_log('Add_Room', 'Room', $result);
	...
	public function edit(){
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					action_log('Edit_Room', 'Room', $post_data['id']);
					$this->success ( "操作成功！",U('index'));
				}else{
					$error = $this->Model->getError();
					$this->error($error ? $error : "操作失败！");
