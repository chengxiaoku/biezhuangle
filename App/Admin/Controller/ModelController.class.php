...
class ModelController extends AdminCoreController {
	...
	public function build() {
		if (IS_POST) {
			...
			//确认生成到哪个模块去，默认为Admin模块
			$this->ModelInfo['modulename'] = I("post.build_tpl_module");
	...
	protected function build_action() {
		...
		$file = APP_PATH . $this->ModelInfo ['modulename']."/Controller/" . $this->ModelInfo ['name'] . "Controller.class.php";
		//停止视图布局
		layout ( false );
	...
	protected function build_model() {
		//设置生成文件的路径 末前支持在Admin模块下的生成
		$file =APP_PATH . $this->ModelInfo ['modulename']. "/Model/" . $this->ModelInfo ['name'] . "Model.class.php";
		//停止视图布局
		layout ( false );
		//模型的配置信息变量替换
		$this->assign ( 'ModelInfo', $this->ModelInfo );
	...
	protected function build_tpl_index() {
		//设置生成文件的路径 末前支持在Admin模块下的生成
		$path = APP_PATH . $this->ModelInfo ['modulename']."/View/" . $this->ModelInfo ['name'] . "/";
	...
	public function getAllModule(){
		$list = M("module")->field("id,name")->select();
		$list[] = array("id"=>0,"name"=>"Admin");
